...
const form = {
  ...
  /*
   ...
   rules: Array [{name:name,rule:[],msg:[]},{name:name,rule:[],msg:[]}]
  		name:name 属性=> 元素的名称
  		rule:字符串数组 ["required","isMobile","isEmail","isCarNo","isIdCard","isBankCard","isAmount","isNum","isChinese","isEnglish",isEnAndNo","isSpecial","isEmoji",""isDate","isUrl","isSame:key","range:[1,9]","minLength:9","maxLength:Number"]
  ...
  */
      ...
      for (let i = 0, length = rule.length; i < length; i++) {
        ...
        switch (ruleItem) {
          ...
          case "isIdCard":
            ...
            break;
          case "isBankCard":
            isError = !form._isBankCard(formData[key]);
            break;
          case "isAmount":
  ...
  _isBankCard: function(value) {
    //银行卡
    return /^([1-9]{1})(\d{15}|\d{12,18})$/.test(value);
  },
